...
def _para(text, style):
    """Un parrafo del PDF, con el texto escapado.

    Paragraph de reportlab no recibe texto plano: parsea un mini-HTML propio
    (<b>, <font>, <br/>). El titulo de un ticket lo escribe cualquiera que abra
    un ticket en GLPI, y ahi eso deja de ser un detalle:

      - "Revisar <font color='red'>esto"  -> tag sin cerrar, revienta el parseo,
        el reporte entero devuelve 500 y no hay forma de sacarlo hasta que
        alguien edite el titulo en GLPI.
      - "Alta <b>URGENTE</b>"             -> el ticket decide como se ve el
        reporte.
      - "Error <urgente> en RAFAM"        -> reportlab se come el <urgente> como
        tag desconocido y el titulo sale incompleto, sin avisar nada.

    Escapando, el texto sale tal cual se escribio. Ningun llamador de esta
    funcion manda markup a proposito: el formato lo pone el `style`, que es el
    segundo parametro."""
...
def generate_report_pdf(
    ...
    story.append(_para(
        f"Soporte Aplicaciones  •  {datetime.now().strftime('%d/%m/%Y %H:%M')}  •  {len(tickets)} ticket(s)",
        st["footer"],
    ))
